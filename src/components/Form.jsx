...
function Form() {
  ...
  useEffect(() => {
    async function fetCityData() {
      try {
        ...
        if(!data.countryCode) return <Message  message="That doen't seem to be a city"/>;
        
        setCityName(data.city || data.locality || "");
        setCountry(data.countryName || "")
        setEmoji(data.countryCode)
        setLocalityInfo(data.localityInfo.administrative?.[4].name);
        console.log(data);
      } catch (error) {
        console.error(error)
      } finally {
  ...
  return (
    <form className={styles.form}>
      ...
      <div className={styles.row}>
        <label htmlFor="date">When did you go to {localityInfo}?</label>
        <input
          id="date"
      ...
      <div className={styles.row}>
        <label htmlFor="notes">Notes about your trip to {localityInfo} from {cityName}</label>
        <textarea
          id="notes"
      ...
      <div className={styles.buttons}>
        ...
        <Button type="back" onClick={() => navigate("/app/form")}>
          &larr; Back
        </Button>
      </div>
    </form>
